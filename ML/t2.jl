...
# ╔═╡ 57b9f4f8-a3f2-4fb3-b87b-c5f123cdc052
begin 
	using PlutoUI
	using Latexify
	TableOfContents()
...
# ╔═╡ 415d6caf-aaf7-4940-a6d0-7ed85832a800
begin
	using Statistics
...
# ╔═╡ 174c73b0-9dc1-11ef-25d2-7919f9b19186
md"""
...
# ╔═╡ 79fcdcb4-229e-4cec-b1b8-8552e1a15a98
md"""
...
# ╔═╡ 65ec2e5c-a5d2-4d83-94f5-58ba9d939c4e
md"""
### Loading the dataset
"""

# ╔═╡ 77c5fb73-df44-4bfc-80b2-2c732ed56e6f
md"""
The image gives an idea of the variety of images in each of the 10 categories.
# ![title](https://pytorch.org/tutorials/_images/cifar10.png)
"""

# ╔═╡ 90eafeb7-4462-48c6-9d67-6c55444d563d
md"""
Again, we'll get the data from the MLDatasets repository. 
"""

# ╔═╡ 6140cbd4-7e52-4571-9639-ab22723d3b79
begin
...
# ╔═╡ 8efdb583-1433-4c6e-bae2-cbabfd818db6
md"""
...
# ╔═╡ 42a5ebbb-7050-4dfb-a9a5-0713231fad2c
image(x) = colorview(RGB, permutedims(x, (3, 2, 1)))

# ╔═╡ 72e33e59-cdad-434f-81f3-45116934ac14
begin
...
# ╔═╡ 53dd3b0f-2bb4-4b3f-92ca-5d06839cc5b6
md"""
### Reshape data for training with `flux`
"""

# ╔═╡ a1603753-b03c-443d-bd69-e58c41a4c9a3
md"""
 We can now
...
# ╔═╡ 15e7d697-ea42-4ca8-8ff8-ad864d7b48d2
md"""
!!! info "Task 1 Updated"
	Partition train_x into training and validation parts, along the lines done for the MNIST example.
"""

# ╔═╡ 6d0a779f-5d08-48de-a7cb-04cce6b6c2f5
md"""
# Task 1:
"""

# ╔═╡ 0a924926-5efe-4894-a012-e632da9e8d7c
begin
	train = [(train_x[:, :, :, i], train_labels[:, i])
		for i in partition(1:49000, 100)] |> gpu

	validate = [(train_x[:, :, :, i], train_labels[:, i])
		for i in partition(49000:50000, 100)] |> gpu
end

# ╔═╡ 8b4106f8-02be-4e32-8f5c-c8ff908c3480
md"""
...
# ╔═╡ 84af8acb-4277-4e58-bac0-407c4d77022c
md"""
...
# ╔═╡ d3753cdd-3175-458b-a6af-093ab1d31322
md"""
!!! error "Model fix"
...
# ╔═╡ 872f8a6d-8002-4481-b5b8-46b55561219c
model = Chain(
...
# ╔═╡ f0e1c6c8-de0f-4934-8267-5152c7a9efc8
model_a = Chain(
    Conv((5,5), 3=>16, pad=SamePad(), relu),
...
# ╔═╡ b0b4c76c-a6f1-4426-8c45-1261846863f2
# Model_b
model_b = Chain(
    Conv((5,5), 3=>16, pad=SamePad(), relu),
    MaxPool((2,2)),
...
# ╔═╡ 47b4ca52-6028-4924-a5bf-3c8f57233764
begin
    ...
    # Model A
    loss_a(x, y) = sum(crossentropy(model_a(x), y))
    optimiser_a = Momentum(0.01)

    # Model B
    loss_b(x, y) = sum(crossentropy(model_b(x), y))
    optimiser_b = Momentum(0.01)
end;

# ╔═╡ 7184f4e4-9355-4103-999b-e372cea01379
md"""
!!! info "Task 2"
...
# ╔═╡ 23220bdd-75ff-46a1-9ea1-2caf67f3243c
md"""
### Test network 
...
# ╔═╡ d00caa04-25fe-4c15-befe-18a0d87a6f11
with_terminal() do
    x = rand(Float32, 32, 32, 3, 1)  # Example input
    model = Chain(
        Conv((5,5), 3=>16, pad=SamePad(), relu),
        MaxPool((2,2)),
        Conv((3,3), 16=>16, pad=SamePad(), relu),
        MaxPool((2,2)),
        Conv((5,5), 16=>8, pad=SamePad(), relu),
        MaxPool((2,2)),
        Flux.flatten
    ...
    output = model(x)
    println("Output size after flattening: ", size(output))
end

# ╔═╡ 908a910c-8c77-4b3b-a277-9cf423cc3f56
md"""
...
# ╔═╡ d97773f4-f2dc-4ad9-adfc-657685f5c20b
md"""
...
# ╔═╡ f8a2eb44-6fa5-473d-86f9-d5b6ad247643
accuracy(x, y) = mean(onecold(model(x), 0:9) .== onecold(y, 0:9))

# ╔═╡ 8dbd0801-467a-435d-8656-3eea2feaf28f
accuracy_a(x, y) = mean(onecold(model_a(x), 0:9) .== onecold(y, 0:9))

# ╔═╡ e133a9f8-b375-4280-a88a-9eafd7c14934
accuracy_b(x, y) = mean(onecold(model_b(x), 0:9) .== onecold(y, 0:9))

# ╔═╡ 98311fc3-47f5-4620-8c4c-c49dd7e1544c
begin
	validate_x = cat([batch[1] for batch in validate]..., dims=4)
	validate_y = hcat([batch[2] for batch in validate]...)
end

# ╔═╡ 31b03b80-f477-4598-8a94-b1acd2ca9e20
md"""
### Training
...
# ╔═╡ e7d026c2-9c9b-4ed8-b479-cd3791765e26
with_terminal() do
    correct = []
	epochs = 100
    for epoch = 1:epochs
...
# ╔═╡ ef474bef-8323-450c-b79a-307b4eae69e0
with_terminal() do
    correct_a = []
    epochs = 100
    for epoch = 1:epochs
...
# ╔═╡ a4be602e-8813-401c-b027-0e25d5e56b76
with_terminal() do
    correct_b = []
	epochs = 100
    for epoch = 1:epochs
        for d in train
            gradients = gradient(Flux.params(model_b)) do
                l = loss_b(d...)
            end
            update!(optimiser_b, Flux.params(model_b), gradients)
        end
        acc = accuracy_b(validate_x, validate_y)
        push!(correct_b, acc)
        println("Model_b - Epoch $epoch Accuracy: $acc")
    end
    plot(correct_b, ylim=(0.0, 0.75), 
        legend=:none, title="Accuracy", xlabel="epoch", ylabel="proportion correct")
end

# ╔═╡ cc848b22-25c8-420c-be4d-15c5a86cf064
md"""
#### Task 2: Comments
...
| **Model A**      | 638                   | 65.53                  | 68.33                 | Improved accuracy but slower due to added complexity.      |
| **Model B**      | 355                   | 63.84                  | 65.53                 | Accuracy slightly the same; minimal impact.                 |
"""

# ╔═╡ 87f3d07e-a92a-41ff-af86-90f42b929dac
md"""
### Testing the network
...
# ╔═╡ b142050d-cbcc-4b4a-9a17-e33c42d76ce2
md"""
!!! info "Task 3"
	Partition the test set similarly to the training set.
"""

# ╔═╡ ccba1210-9476-489b-bc55-c9e6db4236c6
begin
	...
	test = [(test_x[ :,:,:,i], test_labels[:,i])
		for i in partition(1:10000, 1000)] |> gpu;
end

# ╔═╡ 330db546-2ed0-479c-9e0f-848c943acf95
begin
    using DataFrames

    rand_indices = rand(1:size(test_x, 4), 10)
    rand_test = test_x[:, :, :, rand_indices]
    rand_labels = test_y[rand_indices]

    rand_test = cu(rand_test)
    sample_preds = Array(model(rand_test))

    class_labels = ["airplane", "automobile", "bird", "cat", "deer", "dog", "frog", "horse", "ship", "truck"]
    predicted_indices = onecold(sample_preds, 0:9)
    predicted_classes = [class_labels[i] for i in predicted_indices]  

    df = DataFrame(
        "Classes/Actual" => class_labels,
        [predicted_classes[i] => round.(sample_preds[:, i], digits=2) for i in 1:10]...;  
        makeunique=true
    )
    ...
    <div style="overflow-x: auto;">
        <table>
            <thead>
                <tr>
                    $(join(["<th>$(col)</th>" for col in names(df)], " "))
                </tr>
            </thead>
            <tbody>
                $(join(["<tr>" * join(["<td>$(df[row, col])</td>" for col in names(df)], " ") * "</tr>" for row in 1:size(df, 1)], "\n"))
            </tbody>
        </table>
    </div>
    """)
...
# ╔═╡ 7092ed20-4de2-44fe-a03a-466e70544606
function test_accuracy(model, test_data)
    total_correct = 0
    total_samples = 0

    for (x, y) in test_data
        preds = model(x)
        total_correct += sum(onecold(preds, 0:9) .== onecold(y, 0:9))
        total_samples += size(x, 4)  # Number of images in the batch
    end
...
# ╔═╡ f262ffe7-e069-46fc-b145-7256f663b448
begin
	# For the baseline model
	acc_baseline = test_accuracy(model, test)
	println("Baseline Model Test Accuracy: $(acc_baseline)")
	
	# For Model A
	acc_model_a = test_accuracy(model_a, test)
	println("Model A Test Accuracy: $(acc_model_a)")
	
	# For Model B
	acc_model_b = test_accuracy(model_b, test)
	println("Model B Test Accuracy: $(acc_model_b)")
end

# ╔═╡ a0ad39ba-c58c-44a5-9e99-ebee7bbad6fe
md"""
!!! info "Task 4"
...
# ╔═╡ a0e88e97-8119-4bc4-b5fd-04ead6cb83e2
md"""
...
# ╔═╡ caca5ad6-7fcd-4080-9510-47b7121a8dc0
md"""
#### Baseline model:
"""

# ╔═╡ 238bd99c-360f-4413-9320-fdf085db85f6
md"""
!!! tip
...
# ╔═╡ bbec2c8b-0801-49c6-b062-e7d60a7bc9bd
actual_classes = [class_labels[rand_labels[i] + 1] for i in 1:10]

# ╔═╡ ac280189-2db6-4bda-8ad6-c0163dd6f137
@bind inx Slider(1:10, default=1)

# ╔═╡ 59cad463-0c3c-421a-ac2a-6f1bc8f9661f
begin
    selected_image = rand_test[:, :, :, inx]
    selected_label_index = rand_labels[inx]
    selected_label = class_labels[selected_label_index + 1]

    selected_pred = sample_preds[:, inx]
    selected_pred_index = onecold(selected_pred, 0:9)
    selected_pred_class = class_labels[selected_pred_index + 1]

	display_image = colorview(RGB, permutedims(cpu(selected_image), (3, 1, 2)))
    plt = plot(display_image, axis=false, title="Predicted: $selected_pred_class\nActual: $selected_label")

    # display(plt)
end

# ╔═╡ b89696cd-4266-440c-ac9a-9d8ecaeafbe1
md"""
#### Model a:
"""

# ╔═╡ a24f7bb5-b996-4d3b-a22b-f8108414a623
begin
    sample_preds_a = Array(model_a(rand_test))
    predicted_indices_a = onecold(sample_preds_a, 0:9)
    predicted_classes_a = [class_labels[i + 1] for i in predicted_indices_a]  

    df_a = DataFrame(
        "Classes/Actual" => class_labels,
        [predicted_classes_a[i] => round.(sample_preds_a[:, i], digits=2) for i in 1:10]...;  
        makeunique=true
    )
    ...
    <div style="overflow-x: auto;">
        <table>
            <thead>
                <tr>
                    $(join(["<th>$(col)</th>" for col in names(df_a)], " "))
                </tr>
            </thead>
            <tbody>
                $(join(["<tr>" * join(["<td>$(df_a[row, col])</td>" for col in names(df_a)], " ") * "</tr>" for row in 1:size(df_a, 1)], "\n"))
            </tbody>
        </table>
    </div>
    """)
...
# ╔═╡ 62e668cb-8196-4fd7-96a3-e0cc907cb25e
md"""
#### Model b:
"""

# ╔═╡ df6ae4b8-b95d-42ef-8856-fa6502367bc6
begin
    sample_preds_b = Array(model_b(rand_test))
    predicted_indices_b = onecold(sample_preds_b, 0:9)
    predicted_classes_b = [class_labels[i + 1] for i in predicted_indices_b]  

    df_b = DataFrame(
        "Classes/Actual" => class_labels,
        [predicted_classes_b[i] => round.(sample_preds_b[:, i], digits=2) for i in 1:10]...;  
        makeunique=true
    )
    ...
    <div style="overflow-x: auto;">
        <table>
            <thead>
                <tr>
                    $(join(["<th>$(col)</th>" for col in names(df_b)], " "))
                </tr>
            </thead>
            <tbody>
                $(join(["<tr>" * join(["<td>$(df_b[row, col])</td>" for col in names(df_b)], " ") * "</tr>" for row in 1:size(df_b, 1)], "\n"))
            </tbody>
        </table>
    </div>
    """)
...
# ╔═╡ 74fe3571-6a76-4271-8df8-97aa1d8c1480
md"""
...
# ╔═╡ 2b379490-233f-4bfe-9b5e-c53431f00f1d
md"""
## Overall accuracy
...
# ╔═╡ 575442fa-741d-4f38-ac2b-0b40fb52b990
md"""
#### Baseline Model:
"""

# ╔═╡ 8bc8aa4b-8379-4ffa-bc87-b242ad7f6b85
begin
    class_correct = zeros(10)
    class_total = zeros(10)

    for i in 1:length(test)
        # Get predictions and labels for the batch
        preds = model(test[i][1])       # preds: (10, batch_size) on GPU
        lab = test[i][2]                # lab: OneHotMatrix on GPU

        # Move predictions to CPU and get predicted class indices
        pred_classes = onecold(cpu(preds), 0:9)  # Convert to class indices

        actual_classes = Int.(cpu(lab.indices)) .- 1  # Convert UInt32 indices to Int and adjust for 0-based indexing

        # Update counts
        for c in 0:9
            idx = (actual_classes .== c)
            class_total[c + 1] += sum(idx)
            class_correct[c + 1] += sum(pred_classes[idx] .== c)
        end
    end
end


# ╔═╡ f146d164-46e4-4334-9ff2-aaec533b374e
DataFrame(accuracy=(class_correct ./ class_total), class=classes)

# ╔═╡ c718b170-602f-4418-b3e1-5ac57a45e6b1
round(mean([accuracy(test[i]...) for i in 1:10]), digits=3)

# ╔═╡ f5b29cc1-b485-4a84-b664-7087a4620025
md"""
#### Model a:
"""

# ╔═╡ 3b2afac1-7ad9-4f76-9c9d-2140d009ab04
begin
    class_correct_a = zeros(10)
    class_total_a = zeros(10)

    for i in 1:length(test)
        preds_a = model_a(test[i][1])
        lab = test[i][2]

        pred_classes_a = onecold(cpu(preds_a), 0:9)

        actual_classes = Int.(cpu(lab.indices)) .- 1

        for c in 0:9
            idx = (actual_classes .== c)
            class_total_a[c + 1] += sum(idx)
            class_correct_a[c + 1] += sum(pred_classes_a[idx] .== c)
        end
    end
end


# ╔═╡ 53c338ec-a909-4a9b-888f-8b5c20227042
df_model_a = DataFrame(accuracy=(class_correct_a ./ class_total_a), class=classes)

# ╔═╡ 9d44a346-55c9-4828-9182-89c518a771c0
mean_accuracy_a = round(mean([accuracy_a(test[i]...) for i in 1:length(test)]), digits=3)

# ╔═╡ 4588a786-047f-4659-af86-8c5f4da54e25
md"""
#### Model b:
"""

# ╔═╡ d2729b9b-3416-4686-999b-92936ecab616
begin
    class_correct_b = zeros(10)
    class_total_b = zeros(10)

    for i in 1:length(test)
        preds_b = model_b(test[i][1])
        lab = test[i][2]

        pred_classes_b = onecold(cpu(preds_b), 0:9)

        actual_classes = Int.(cpu(lab.indices)) .- 1

        for c in 0:9
            idx = (actual_classes .== c)
            class_total_b[c + 1] += sum(idx)
            class_correct_b[c + 1] += sum(pred_classes_b[idx] .== c)
        end
    end
end

# ╔═╡ 726e0b10-90ea-4abc-8ebc-95ffa015ea2f
df_model_b = DataFrame(accuracy=(class_correct_b ./ class_total_b), class=classes)

# ╔═╡ e7558e91-373e-4368-ad83-f7b2e2526c4d
mean_accuracy_b = round(mean([accuracy_b(test[i]...) for i in 1:length(test)]), digits=3)
...
manifest_format = "2.0"
project_hash = "a3ed37e3b23bd3576a6e65d9b5ae4f29e8834624"
